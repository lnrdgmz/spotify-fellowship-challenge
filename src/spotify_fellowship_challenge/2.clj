...
;; We'll iterate through the encoded string and build up a stack of strings
;; that will be expanded later. Let's define some helper functions first.
...
;; of the stack). Nested strings need to be expanded further, so we add the newly
;; expanded string to the next item in the stack. If the stack has only one item
;; (or none), there's nothing to do, so we just return it unchanged.
...
;; Now we can define our function. We'll iterate through the string, adding items
;; to the stack when we encounter a number and a `[`, and expand strings when we
;; hit a `]`
